...
contract iNFT is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(_mintAmount < 3 ,'Cant mint more than 2 nfts');

        require(msg.value >= cost * _mintAmount);
        uint256 supply = totalSupply();

        require(supply + _mintAmount <= maxSupply ); //cant mint more tokens than available
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns(string memory){
        ...
        baseURI, _tokenId.toString(),baseExtension
        )));
    }
